...
| `user-prompt-submit.sh` | UserPromptSubmit | legacy `<<bus>>` sentinel from an old peer → drain into `additionalContext` |
| `session-end.sh` | SessionEnd | `bus gc` (sweep + prune) so `bus.db` stays bounded |
