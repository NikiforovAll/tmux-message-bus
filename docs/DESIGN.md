...
- **Receive** (Stop-hook drain): when the peer finishes a turn, the Stop hook drains its mail atomically
  ...
  ```
  and injects the bodies as context (Stop hook returns `decision:block` + reason = the messages, so the agent keeps going and acts on them). Idle peer = no Stop event, so the doorbell is what creates the turn (consistent with the drain choice).
  ...
  - **Loop guard (validated):** drain-and-clear means the *next* Stop fires on an empty mailbox and returns no decision, so the agent is allowed to terminate. Block exactly once per batch; never block when zero rows were drained.
