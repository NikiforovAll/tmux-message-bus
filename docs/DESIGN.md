...
- **`decision:block` re-prompts: CONFIRMED.** A Stop hook emitting `{decision:block, reason}` makes the agent continue the turn and act on the reason. The load-bearing assumption holds. *(Superseded in the shipped adapter: the block form is surfaced twice — see "Why not `decision:block`" under Flows -> Receive — so the Stop hook now re-prompts via `additionalContext`, which continues the turn with a single emission.)*
